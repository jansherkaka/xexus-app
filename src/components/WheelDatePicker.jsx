...
function WheelColumn({ items, selectedIndex, onSelect, align }) {
  ...
  return (
    <div className={`wheel-col wheel-col--${align}`} ref={ref} onScroll={handleScroll}>
      ...
      {items.map((item, i) => {
        ...
        return (
          <div
            key={item}
            className={`wheel-col__item wheel-col__item--d${Math.min(distance, 2)}`}
...
export default function WheelDatePicker({ value, onChange, minYear = 1940 }) {
  ...
  return (
    <div className="wheel-picker">
      ...
      <WheelColumn items={MONTHS} selectedIndex={month} onSelect={setMonth} align="month" />
      <WheelColumn items={days} selectedIndex={clampedDay - 1} onSelect={(i) => setDay(i + 1)} align="day" />
      <WheelColumn items={years} selectedIndex={year - minYear} onSelect={(i) => setYear(minYear + i)} align="year" />
    </div>
  );
}
